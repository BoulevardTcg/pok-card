...
router.get('/profile', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Erreur users:', error);
    res.status(500).json({
...
router.post(
  ...
  authenticateToken,
  [
    body('cardId').notEmpty().withMessage('ID de la carte requis'),
    body('cardName').notEmpty().withMessage('Nom de la carte requis'),
    body('cardImage').notEmpty().withMessage('Image de la carte requise'),
    body('cardSet').notEmpty().withMessage('Série de la carte requise'),
  ],
  async (req: Request, res: Response) => {
    try {
      // Vérifier les erreurs de validation
