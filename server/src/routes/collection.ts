...
router.post(
  ...
  authenticateToken,
  [
    body('cardId').isString().notEmpty().withMessage("L'ID de la carte est obligatoire"),
    body('cardName').isString().notEmpty().withMessage('Le nom de la carte est obligatoire'),
    body('quantity')
      .optional()
      .isInt({ min: 1 })
      .withMessage('La quantité doit être supérieure à 0'),
    body('condition').optional().isString().withMessage("L'état doit être une chaîne"),
    body('language').optional().isString().withMessage('La langue doit être une chaîne'),
  ],
