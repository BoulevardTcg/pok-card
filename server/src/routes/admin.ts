...
const fileFilter = (req: Request, file: Express.Multer.File, cb: multer.FileFilterCallback) => {
  ...
  if (allowedTypes.includes(file.mimetype) || allowedExtensions.includes(ext)) {
    ...
    cb(
      new Error(
        `Type de fichier non autorisé: ${file.mimetype} (${ext}). Utilisez JPG, PNG, GIF ou WebP.`
      )
    );
  }
};
...
router.post(
  ...
  [
    ...
    body('note').optional().isString().isLength({ max: 500 }).withMessage('Note trop longue'),
  ],
  async (req: Request, res: Response) => {
    try {
      ...
      const { orderId } = req.params;
      const { carrier, trackingNumber, note } = req.body as {
        carrier: Carrier;
...
router.patch(
  ...
  [
    ...
    body('trackingUrl').optional().isString().trim().isURL().withMessage('URL de suivi invalide'),
  ],
  async (req: Request, res: Response) => {
    try {
      const errors = validationResult(req);
      ...
      const nextTrackingNumber: string | null =
        trackingNumber ?? existingOrder.trackingNumber ?? null;
      const resolvedTrackingUrl: string | null =
        trackingUrl ??
        (nextTrackingNumber
          ? buildTrackingUrl(nextCarrier ?? Carrier.OTHER, nextTrackingNumber)
          : (existingOrder.trackingUrl ?? null));
      ...
      await addOrderEvent(
        ...
        `Statut: ${existingOrder.status} → ${status}`,
        req.user?.userId
      );
      if (status === 'SHIPPED') {
...
router.patch(
  '/users/:userId',
  [body('isAdmin').optional().isBoolean(), body('isVerified').optional().isBoolean()],
  async (req: Request, res: Response) => {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
