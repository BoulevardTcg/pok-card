...
export const validateSecurityConfig = () => {
  ...
  if (!process.env.JWT_SECRET || process.env.JWT_SECRET.length < SECURITY_CONFIG.JWT.SECRET_MIN_LENGTH) {
    errors.push(`JWT_SECRET doit faire au moins ${SECURITY_CONFIG.JWT.SECRET_MIN_LENGTH} caractères`)
  }
  
  if (!process.env.JWT_REFRESH_SECRET || process.env.JWT_REFRESH_SECRET.length < SECURITY_CONFIG.JWT.SECRET_MIN_LENGTH) {
    errors.push(`JWT_REFRESH_SECRET doit faire au moins ${SECURITY_CONFIG.JWT.SECRET_MIN_LENGTH} caractères`)
  }
