...
const toProductResponse = (product: ProductWithRelations) => {
  ...
  return {
    ...
    outOfStock: activeVariants.length === 0 || activeVariants.every((variant) => variant.stock <= 0)
  }
}
...
router.get('/:slug', async (req, res) => {
  try {
    ...
    console.log('✅ Réponse formatée:', { id: response.id, name: response.name, variantsCount: response.variants.length })
