...
function SalesChart({ data, formatPrice }: { data: SalePoint[]; formatPrice: (cents: number) => string }) {
  ...
  const visibleLabels = xLabels.filter((_, i) => i === 0 || i === data.length - 1 || i % Math.ceil(data.length / 6) === 0);

  const handleMouseMove = useCallback((e: React.MouseEvent<SVGSVGElement>, index: number) => {
    const rect = e.currentTarget.getBoundingClientRect();
    setTooltip({ x: e.clientX - rect.left, y: e.clientY - rect.top, data: data[index], visible: true });
    ...
    <div className={styles.chartWrapper}>
      <div className={styles.chartLegend}>
        <div className={styles.legendItem}>
          <span className={styles.legendDot} style={{ background: 'var(--color-accent-secondary)' }} />
          Revenus (€)
        </div>
        <div className={styles.legendItem}>
      ...
      <svg viewBox={`0 0 ${width} ${height}`} className={styles.chartSvg} onMouseLeave={handleMouseLeave}>
        <defs>
        ...
        {yTicksRevenue.map((tick, i) => (
          <g key={i}>
            ...
            <text x={padding.left - 10} y={tick.y + 4} textAnchor="end" className={styles.axisLabel}>{formatPrice(tick.value)}€</text>
          </g>
        ))}

        {visibleLabels.map((label, i) => (
          <text key={i} x={label.x} y={height - padding.bottom + 25} textAnchor="middle" className={styles.axisLabel}>{label.label}</text>
        ))}
      ...
      {tooltip.visible && tooltip.data && (
        <div className={styles.tooltip} style={{ left: Math.min(tooltip.x + 15, 700), top: tooltip.y - 10 }}>
          <div className={styles.tooltipHeader}>{tooltip.data.period}</div>
          <div className={styles.tooltipRow}><span className={styles.tooltipDot} style={{ background: 'var(--color-accent-secondary)' }} /> Revenus: <strong>{formatPrice(tooltip.data.revenue)}€</strong></div>
          <div className={styles.tooltipRow}><span className={styles.tooltipDot} style={{ background: 'var(--color-success)' }} /> Commandes: <strong>{tooltip.data.orders}</strong></div>
          <div className={styles.tooltipRow}><span className={styles.tooltipDot} style={{ background: 'var(--color-warning)' }} /> Articles: <strong>{tooltip.data.items}</strong></div>
        </div>
      )}
    </div>
  );
...
export function AdminReportsPage() {
  ...
  function generateDemoData(period: 'day' | 'week' | 'month') {
    ...
    for (let i = count - 1; i >= 0; i--) {
      ...
      if (period === 'day') { date.setDate(date.getDate() - i); label = date.toLocaleDateString('fr-FR', { day: '2-digit', month: 'short' }); }
      else if (period === 'week') { date.setDate(date.getDate() - i * 7); label = `Sem ${Math.ceil(date.getDate() / 7)}`; }
      else { date.setMonth(date.getMonth() - i); label = date.toLocaleDateString('fr-FR', { month: 'short', year: '2-digit' }); }
  ...
  const formatPriceShort = (cents: number) => cents >= 100000 ? `${(cents / 100000).toFixed(1)}k` : formatPrice(cents);
      ...
      <div className={styles.pageHeader}>
        ...
        <div className={styles.periodSelector}>
          {(['day', 'week', 'month'] as const).map((p) => (
            <button key={p} onClick={() => setPeriod(p)} className={`${styles.periodBtn} ${period === p ? styles.active : ''}`}>
        ...
        <>
          ...
          <div className={styles.statsGrid}>
            <div className={styles.statCard}>
              <div className={styles.statIcon}><EuroIcon /></div>
              <div className={styles.statContent}>
                ...
                <span className={`${styles.statTrend} ${variation.positive ? styles.positive : styles.negative}`}>
            ...
            <div className={styles.statCard}>
              <div className={styles.statIcon}><CartIcon /></div>
              <div className={styles.statContent}>
                <span className={styles.statLabel}>Commandes</span>
                <span className={styles.statValue}>{salesData.total.orders}</span>
                <span className={styles.statSubtext}>Moy. {formatPrice(salesData.total.revenue / (salesData.total.orders || 1))}€/cmd</span>
              </div>
            </div>
            ...
            <div className={styles.statCard}>
              ...
              <div className={styles.statContent}>
                ...
                <span className={styles.statSubtext}>{(salesData.total.items / (salesData.total.orders || 1)).toFixed(1)} art./cmd</span>
              </div>
            </div>

            <div className={styles.statCard}>
              <div className={styles.statIcon}><TrendingIcon /></div>
              <div className={styles.statContent}>
                <span className={styles.statLabel}>Tendance</span>
                <span className={`${styles.statValue} ${variation.positive ? styles.textPositive : styles.textNegative}`}>
                  {variation.positive ? '+' : '-'}{variation.value}%
                </span>
                <span className={styles.statSubtext}>{variation.positive ? 'En croissance' : 'En baisse'}</span>
              </div>
            </div>
          </div>
          ...
          <div className={styles.chartCard}>
            <div className={styles.chartHeader}>
              ...
              <p className={styles.chartSubtitle}>Revenus et commandes sur la période sélectionnée</p>
            </div>
