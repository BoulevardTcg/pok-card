  ...
  async function verifyAndCreateOrder(sessionId: string) {
    try {
      ...
      if (response.ok && data.success) {
        ...
        console.log(`✅ Commande ${data.alreadyCreated ? 'existante' : 'créée'}: ${data.orderNumber}`);
      } else {
    ...
    <div className={styles.container}>
      <div className={styles.successCard}>
        <div className={styles.iconContainer}>
          <div className={styles.checkmark}>
            <svg viewBox="0 0 52 52">
              ...
              <path fill="none" stroke="#D7B56D" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" d="M14 27l8 8 16-16"/>
            </svg>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className={styles.sessionInfo}>
            <p className={styles.sessionLabel} style={{ color: '#ef4444' }}>{error}</p>
          </div>
        ) : null}
