...
function mapFrenchRarityToEffect(card: TradeCard): {
  ...
  const isBase2Series =
    setSeries.includes('base 2') || setSeries.includes('base2') || setSeries.includes('base-2');
...
export function HoloCard({ card, foilMap }: Props & { foilMap?: Map<string, string> | null }) {
  ...
  const { dataRarity, dataSubtypes, dataSupertype, dataTrainerGallery, dataIllusion2 } = useMemo(
    () => mapFrenchRarityToEffect(card),
    [card]
  );
  ...
  const imageUrl = active
    ? card.imagesLarge || card.imagesSmall || ''
    : card.imagesSmall || card.imagesLarge || '';
